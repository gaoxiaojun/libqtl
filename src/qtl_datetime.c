...
static void
rdn_to_ymd(uint32_t rdn, uint16_t *yp, uint16_t *mp, uint16_t *dp, uint16_t *yday) {
    uint32_t Z, H, A, B;
...
yd_to_rdn(uint16_t y, uint16_t d) {
    y--;
    return 365 * y + y/4 - y/100 + y/400 + d;
}

static inline uint32_t
ymd_to_rdn(uint16_t y, uint16_t m, uint16_t d) {
    if (m < 1 || m > 12) {
        y += m / 12;
        m %= 12;
        if (m < 1)
...
void datetime_to_ymd(datetime_t time, uint16_t *year, uint16_t *month, uint16_t *day)
{
    uint32_t rdn = time / TICKS_PER_DAY;
    rdn_to_ymd(rdn, year, month, day, NULL);
}

datetime_t datetime_from_hmsu(uint16_t hour, uint16_t minute, uint16_t second, uint32_t usec) {
...
void datetime_decode(datetime_t dt, uint16_t *year, uint16_t *month, uint16_t *day, uint16_t *hour, uint16_t *minute, uint16_t *second, uint32_t *usec)
{
    ...
    *second  = sod % 60; sod /= 60;
    *minute  = sod % 60; sod /= 60;
    *hour = sod;
}
...
int
datetime_from_iso8601(const char *str, size_t len, datetime_t *dt) {
...
int
datetime_to_tm(const datetime_t dt, struct tm *tmp) {
    uint32_t rdn, sod;

    if (dt < dt - EPOCH_DATE_TIME) return 1;
...
static size_t
timestamp_format_internal(char *dst, size_t len, const datetime_t dt, int32_t zone_offset, const int precision) {
    unsigned char *p;
    uint64_t sec;
    uint32_t rdn, v, nsec;
    uint16_t y, m, d;
